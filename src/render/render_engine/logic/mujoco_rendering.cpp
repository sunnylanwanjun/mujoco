...
protected:
    ...
    void _Init() override {
        ...
        _frameBuffer.Create(renderPass);

        {
            _texture.UploadWithFile(TEXTURE_PATH.c_str());
        }
    ...
    void _UnInit() override {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            vkDestroySemaphore(_device.GetDevice(), renderFinishedSemaphores[i], nullptr);
            vkDestroySemaphore(_device.GetDevice(), imageAvailableSemaphores[i], nullptr);
            vkDestroyFence(_device.GetDevice(), inFlightFences[i], nullptr);
        }
    }
    ...
    void createRenderPass() {
        ...
        depthAttachment.format = _frameBuffer.findDepthFormat();
    ...
    void createUniformBuffers() {
        ...
        _uniformBuffers.resize(MAX_FRAMES_IN_FLIGHT, _device);
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    ...
    void createCommandBuffers() {
        ...
        if (vkAllocateCommandBuffers(_device.GetDevice(), &allocInfo, commandBuffers.data()) != VK_SUCCESS) {
    ...
    void createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(_device.GetDevice(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
                vkCreateSemaphore(_device.GetDevice(), &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS ||
                vkCreateFence(_device.GetDevice(), &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS) {
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        _uniformBuffers[currentImage].WriteWithPersistentMap(&ubo, sizeof(ubo));
    }

    void _Update() {
        vkWaitForFences(_device.GetDevice(), 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX);

        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(_device.GetDevice(), _swapChain.GetSwapChain(), UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
        ...
        vkResetFences(_device.GetDevice(), 1, &inFlightFences[currentFrame]);
        ...
        if (vkQueueSubmit(_device.GetGraphicsQueue(), 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
